...
import ProCapStatus from "@components/RulesPage/ProCapStatus";
...
In-Season Free Agent Signing is started by a team posting the player they wish to sign in the #siba-transactions channel in Discord. Then that player is available until the next sim or for 6 hours, whichever period is shorter, to give any other teams a chance of signing him. After the waiting period, the commissioner will use the website <ExteralLink href="http://www.random.org/" label="True Random Number Generator" /> to calculate the number. Three random numbers will be average together to get the free agent number of interest.
